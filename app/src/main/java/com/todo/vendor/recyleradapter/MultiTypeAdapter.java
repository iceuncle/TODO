...
public class MultiTypeAdapter extends BaseViewAdapter<Object> {
    ...
    public void remove(Object item) {
        int pos = mCollection.indexOf(item);
        mCollectionViewType.remove(pos);
        super.remove(pos);
    }

    public void removeAndAdd(Object item, int addViewType) {
        int pos = mCollection.indexOf(item);
        mCollectionViewType.remove(pos);
        mCollection.remove(pos);
        mCollection.add(pos, item);
        mCollectionViewType.add(pos, addViewType);
        notifyItemChanged(pos);
    }

    public void removeAndAdd(Object removeItem, Object addItem, int addViewType) {
        int pos = mCollection.indexOf(removeItem);
        mCollectionViewType.remove(pos);
        mCollection.remove(pos);
        mCollection.add(pos, addItem);
        mCollectionViewType.add(pos, addViewType);
        notifyItemChanged(pos);
    }

    public <T> T getMultiItem(int pos) {
        return ((T) mCollection.get(pos));
    }
